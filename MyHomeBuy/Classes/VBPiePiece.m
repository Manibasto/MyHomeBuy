...
- (void) setPath:(CGPathRef)path {
    ...
    if (_labelsPosition != VBLabelsPositionNone) {
        ...
        //if (labelSize.width > 130) {
            //labelSize.width = 130;
        //}
        [CATransaction begin];
        [CATransaction setDisableActions:YES];
        
       // [self.label setBackgroundColor: [UIColor blackColor].CGColor];
        ...
        int count = 0;
        NSArray *str = @[@"Budget",@"Let's Start Looking",@"Offer to Purchase",@"Legal",@"Contract",@"Property Settlement",@"Moving In"];
        for (int i = 0; i<[str count]; i++) {
            if([str[i] isEqualToString:_pieceName]){
                count = i;
                break;
            }
        }
       ...
       // NSLog(@"-------------------------");
        //NSLog(@"strName ===== %@  x== %f and  y ==== %f",_pieceName,center.x,center.y);
        //NSLog(@"-------------------------");
